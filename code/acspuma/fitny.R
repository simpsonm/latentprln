...
standat <- puma_standats$NY$y2015
...
save(standat.ny, file = paste('standat.', state, '.RData', sep = ""))

set.seed(67324923)

fit0 <- stan("../models/prln_manytracts.stan",
             data = standat.ny, chains = 1, iter = 1)

fit <- stan(fit = fit0, data = standat.ny, chains = 4, cores = 4,
