...
standat <- puma_standats$CO$y2015
...
save(standat.co, file = paste('standat.', state, '.RData', sep = ""))

set.seed(9812391)

fit0 <- stan("../models/prln_manytracts.stan",
             data = standat.co, chains = 1, iter = 1)

fit <- stan(fit = fit0, data = standat.co, chains = 4, cores = 4,
