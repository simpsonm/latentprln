...
standat <- puma_standats$MO$y2015
...
save(standat.mo, file = paste('standat.', state, '.RData', sep = ""))

set.seed(812431923)

fit0 <- stan("../models/prln_manytracts.stan",
             data = standat.mo, chains = 1, iter = 1)

fit <- stan(fit = fit0, data = standat.mo, chains = 4, cores = 4,
