...
standat <- puma_standats$MT$y2015
...
save(standat.mt, file = paste('standat.', state, '.RData', sep = ""))

set.seed(6713424)

fit0 <- stan("../models/prln_manytracts.stan",
             data = standat.mt, chains = 1, iter = 1)

fit <- stan(fit = fit0, data = standat.mt, chains = 4, cores = 4,
