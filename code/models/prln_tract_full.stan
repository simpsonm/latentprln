...
transformed data {
  ...
  vector[nbin + 2 + nquant + nshare + 1] tract_full_ests;
  vector<lower = 0>[nbin + 2 + nquant + nshare + 1] tract_full_ses;
  int<lower = 1, upper = nbin - 1> nalpha;
  int<lower = 1, upper = nbin - 1> nunif;

  nest = nbin + 2 + nquant + nshare + 1;
  ...
  median_cdf_est = 0.5;
  tract_full_ests =
    append_row(append_row(append_row(append_row(append_row(bin_est, mean_est),
						median_cdf_est),
				     quant_cdf_est),
			  share_est),
	       gini_est);
  ...
  tract_full_ses =
    append_row(append_row(append_row(append_row(append_row(bin_se, mean_se),
						median_cdf_se),
				     quant_cdf_se),
			  share_se),
	       gini_se);
...
transformed parameters {
...

  {
    ...
    for(j in 1:nquant){
      ...
      idx2 = idx1 + 1;
      if(idx1 <= nunif){
	tract_true_values[nbin + 2 + j] = sum(pknot[1:idx1]) -
	...
	tract_true_values[nbin + 2 + j] = sum(pknot[1:(idx1-1)]) +
    ...
    // income shares
    for(j in 1:nshare){
      tract_true_values[nbin + 2 + nquant + j] =
    ...
    // gini coefficient
    tract_true_values[nbin + 2 + nquant + nshare + 1] =
