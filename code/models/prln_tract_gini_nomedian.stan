...
transformed data {
  ...
  vector[nbin + 1 + 1] tract_full_ests;
  vector<lower = 0>[nbin + 1 + 1] tract_full_ses;
  ...
  tract_full_ests = append_row(append_row(bin_est, mean_est), gini_est);

  tract_full_ses = append_row(append_row(bin_se, mean_se), gini_se);
}
parameters {
  simplex[nbin] pknot;
