...
transformed data {
  ...
  vector[nbin + 2 + nquant + 1] tract_full_ests;
  vector<lower = 0>[nbin + 2 + nquant + 1] tract_full_ses;
  int<lower = 1, upper = nbin - 1> nalpha;
  int<lower = 1, upper = nbin - 1> nunif;

  nest = nbin + 2 + nquant + 1;
...
transformed parameters {
...

  {
    ...
    // gini coefficient
    tract_true_values[nbin + 2 + nquant + 1] =
