...
transformed data {
  ...
  vector[nbin + 2] tract_full_ests;
  vector<lower = 0>[nbin + 2] tract_full_ses;
  ...
  median_cdf_est = 0.5;
  tract_full_ests = append_row(append_row(bin_est, mean_est),
			       median_cdf_est);

  median_cdf_se = median_se * bin_est[nunif] / (knots[nunif + 1] - knots[nunif]);
  tract_full_ses = append_row(append_row(bin_se, mean_se),
			      median_cdf_se);
}
parameters {
  simplex[nbin] pknot;
